...
__description__ = '''\
  ...
  Cipher__Hall | v.''' + __version__ + '''
  Author: warner
  Github: https://github.com/kevinscaria809
  ___________________________________________
'''
...
def update():
    success = False
    git_rep = "git://github.com/kevinscaria809/Cipher__Hall.git"
    tool_path = modulePath()
    
    if not os.path.exists(os.path.join(tool_path, ".git")):
        errMsg = "not a git repository. Please checkout the 'Cipher__Hall' repository "
        exit(write(errMsg))
    else:
        infoMsg = "updating Cipher__Hall to the latest development version from the "
        infoMsg += "GitHub repository"
        write(infoMsg)

        debugMsg = "Cipher__Hall will try to update itself using 'git' command"
        write(debugMsg)
        ...
        if success:
            ...
            if "Not a git repository" in stderr:
                errMsg = "not a valid git repository. Please checkout the 'Cipher__Hall' repository "
                errMsg += "from GitHub (e.g. 'git clone --depth 1 https://github.com/kevinscaria809/Cipher__Hall.git Cipher__Hall')"
                exit(write(errMsg))
            else:
    ...
    if not success:
        if platform.system() == 'Windows':
            ...
            infoMsg += "https://github.com/kevinscaria809/Cipher__Hall/archive/master.zip"
        else:
...
def main():
    ...
    try:
        ...
        if domain:
            ...
            exit(write('error arguments: use cipher__Hall -h to help'))
